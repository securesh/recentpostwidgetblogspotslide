...
// litle modify by http://pcalias.com/
...
function makeSlider(config) {
	...
	var defaults = {
		...
		thumbHeight: 250, // Width of post thumbnail in pixels
		squareThumb: true, // Thumbnail mode: Square mode or use the scaled thumbnail with original width and height ratio
		noThumb: "http://lh3.googleusercontent.com/-FkVR_3lgSVo/VNw1V6tzP5I/AAAAAAAAAnc/DMEQuX-SJd4/s2048/What-is-a-Safe-and-Normal-CPU-Temperature.jpg", // Fallback thumbnail for no picture posts
	...
	$.get(defaults.url + '/feeds/posts/summary/' + (defaults.labelName === null ? "" : '-/' + defaults.labelName.replace(/\,(\s+)?/g,"/")) + '?alt=json-in-script&max-results=' + defaults.numPost, {}, function(json) {
		if (json.feed.entry !== undefined) {
			...
			for (var i = 0, len = entry.length; i < len; i++) {
				...
				skeleton += '<div class="slider-item" style="width:' + defaults.containerw + '%">';
				skeleton += (defaults.showThumb && defaults.showDetail) ? '<div class="image-wrapper" style="float: left; width:50%">' + thumb + '</div>' : (defaults.showThumb && !defaults.showDetail) ? '<div class="image-wrapper" style="float: left; width:50%"><a href="' + link + '" title="' + title + '"' + (defaults.newTabLink ? ' target="_blank"' : '') + '>' + thumb + '</a></div>' : '';
				skeleton += (defaults.showDetail) ? '<div class="detail-wrapper" style="float: left; width:50%"><h4><a title="' + title + '" href="' + link + '"' + (defaults.newTabLink ? ' target="_blank"' : '') + '>' + ((defaults.titleLength == "auto") ? title : title.substring(0, defaults.titleLength) + (title.length > defaults.titleLength ? '&hellip;' : '')) + '</a></h4><p>' + summary + '</p></div>' : '';
				skeleton += '</div>';
				$('#' + defaults.containerId).css({
					'width': (defaults.containerw) ? defaults.containerw + '%' : $('#' + defaults.containerId).css('width'),
...
// :) litle modify by pcalias.com , adding the  variable wimage,	himage,	containerw and containerh.
